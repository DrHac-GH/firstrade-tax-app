...
Firstrade証券の取引履歴CSVを取り込み、日本円での**譲渡損益・配当所得・利子所得**を計算するWebアプリケーションです。

## 機能
- **CSVインポート**: Firstradeからダウンロードした `Gain/Loss` および `Account History` レポートのCSVに対応。
...
- **損益計算**:
  ...
  - 損益 (日本円) = 売却額 (円) - 取得費 (円)
- **配当・利子計算**:
  - 受取額 (USD) × 受取日レート = 所得額 (日本円)
  - 外国税額 (USD) × 受取日レート = 外国税額 (日本円)
- **CSVエクスポート**: 計算結果をCSVとして保存できます。

### 1. 起動
フォルダ内にある **`StartApp.command`** をダブルクリックして起動してください。
ターミナルが開き、必要なパッケージのインストール等を自動で行った後、ブラウザでアプリが起動します。
